...
export default function InstagramDashboardSection() {
  ...
  const loadData = async () => {
    try {
      ...
      if (!response.ok || !result.success) {
        throw new Error(result.error || 'Error al cargar datos de Instagram');
      }

      setData(result.data);
    } catch (err: any) {
      console.error('Error cargando Instagram data:', err);
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-center gap-4">
              ...
              <div className="text-center">
                <p className="text-gray-600 dark:text-gray-400 text-sm">Cargando Instagram Analytics...</p>
      ...
      <Card className="bg-white dark:bg-gray-800 border-gray-100 dark:border-gray-700 shadow-[0_2px_8px_rgba(0,0,0,0.04)]">
        <CardContent className="p-8">
          <div className="flex flex-col items-center justify-center text-center">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-purple-500/20 via-pink-500/20 to-orange-400/20 flex items-center justify-center mb-4">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm mb-4 max-w-md">
              {error || 'Conecta tu cuenta de Instagram para ver estadísticas de reputación, comentarios y análisis de sentimiento.'}
            </p>
            <Button
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              {/* Instagram Icon */}
              <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-500 via-pink-500 to-orange-400 flex items-center justify-center shadow-lg">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400 mt-1">
                  <span className="flex items-center gap-1">
                    ...
                    {data.account.followers.toLocaleString()} seguidores
                  </span>
                  <span>•</span>
                  <span>{data.account.total_posts} publicaciones</span>
                </div>
              </div>
            </div>
          ...
          <CardContent className="pt-0">
            <div className="space-y-3">
              {data.top_posts.slice(0, 3).map((post, index) => (
                ...
                >
                  <div className="flex-shrink-0 w-7 h-7 bg-gradient-to-br from-purple-500 via-pink-500 to-orange-400 text-white rounded-lg flex items-center justify-center text-sm font-semibold">
                    {index + 1}
                  </div>
      ...
      {/* Last Sync Footer */}
      {data.account.last_sync && (
        <div className="text-center">
          <span className="text-xs text-gray-400 dark:text-gray-500">
            Última sincronización: {new Date(data.account.last_sync).toLocaleString('es-CO', {
              day: '2-digit',
              month: '2-digit',
              year: 'numeric',
              hour: '2-digit',
