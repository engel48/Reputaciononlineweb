    ...
    <div className="card p-6">
      ...
      <div className="mt-8 h-64">
        <div className="relative h-full w-full">
          ...
          {[0.25, 0.5, 0.75, 1].map((ratio) => (
            <div 
              key={ratio}
              className="absolute left-0 right-0 border-t border-gray-200 dark:border-gray-200"
              style={{ bottom: `${ratio * 100}%` }}
            >
              <span className="absolute -top-3 -left-10 text-xs text-gray-500 dark:text-gray-500">
          ...
          <div className="absolute bottom-0 left-0 right-0 top-0 flex items-end justify-between pr-10">
            ...
            {datosHistoricos.map((valor, index) => (
              <div key={`hist-${index}`} className="flex w-full flex-col items-center">
                <motion.div
                  className="relative w-10 rounded-t bg-gray-300 dark:bg-gray-200"
                  style={{ height: `${(valor / valorMaximo) * alturaMaxima}px` }}
                  initial={{ height: 0 }}
                  animate={{ height: `${(valor / valorMaximo) * alturaMaxima}px` }}
                  transition={{ duration: 0.5, delay: index * 0.1 }}
                >
                  <span className="absolute -top-6 left-1/2 -translate-x-1/2 transform text-xs text-gray-500 dark:text-gray-500">
                    {valor.toLocaleString('es-CO')}
                  </span>
                </motion.div>
                <div className="mt-2 text-xs text-gray-500 dark:text-gray-500">{mesesHistoricos[index]}</div>
              </div>
            ))}
            
            {/* Línea divisoria */}
            <div className="h-full w-px bg-gray-300 dark:bg-gray-200"></div>
      ...
      <div className="mt-6 flex justify-center space-x-6">
        <div className="flex items-center">
          <div className="mr-2 h-3 w-3 rounded-full bg-gray-300 dark:bg-gray-200"></div>
          <span className="text-xs text-gray-600 dark:text-gray-600">Histórico</span>
        </div>
        <div className="flex items-center">
          <div className="mr-2 h-3 w-3 rounded-full bg-primary-500 dark:bg-primary-600"></div>
          <span className="text-xs text-gray-600 dark:text-gray-600">Proyectado</span>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-100">
          <div className="flex items-center">
            ...
            <p className="ml-2 text-sm font-medium text-gray-700 dark:text-gray-600">Periodo Proyectado</p>
          </div>
          <p className="mt-2 text-lg font-semibold text-gray-900 dark:text-gray-900">
        ...
        <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-100">
          <div className="flex items-center">
            ...
            <p className="ml-2 text-sm font-medium text-gray-700 dark:text-gray-600">Crecimiento Proyectado</p>
          </div>
      ...
      <div className="mt-6 rounded-md bg-blue-50 p-4 dark:bg-blue-900/20">
        <h3 className="text-sm font-medium text-blue-700 dark:text-blue-600">Recomendación</h3>
        <p className="mt-1 text-sm text-blue-600 dark:text-blue-700">
