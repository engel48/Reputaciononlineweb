...
/**
 * Dashboard de Threads - Datos consolidados para visualización
 *
 * GET /api/threads/dashboard
 *
 * Retorna:
 * - Métricas del perfil (seguidores, threads, engagement)
 * - Análisis de sentimiento agregado
 * - Menciones y replies recientes
 * - Tendencias temporales
 * - Top threads por engagement
...
export async function GET(request: NextRequest) {
  try {
    console.log('📊 Threads Dashboard: Generando datos...');
    ...
    const threadStats: { [key: string]: any } = {};

    mentions.forEach(m => {
      const threadId = m.metadata?.thread_id;
      if (threadId) {
        if (!threadStats[threadId]) {
          threadStats[threadId] = {
            thread_id: threadId,
            thread_content: m.metadata?.thread_content || 'Sin contenido',
            total_replies: 0,
            positive_replies: 0,
            negative_replies: 0,
            neutral_replies: 0,
        ...
        threadStats[threadId].total_replies++;
        threadStats[threadId].total_likes += (m.likes || 0);
        threadStats[threadId].total_shares += (m.shares || 0);

        if (m.metadata?.sentiment === 'positive') threadStats[threadId].positive_replies++;
        else if (m.metadata?.sentiment === 'negative') threadStats[threadId].negative_replies++;
        else threadStats[threadId].neutral_replies++;
      }
    });

    const topThreads = Object.values(threadStats)
      .map(thread => ({
        ...thread,
        avg_sentiment: thread.total_replies > 0
          ? ((thread.positive_replies - thread.negative_replies) / thread.total_replies) * 100
          : 0
      }))
    ...
    const dashboard = {
      ...
      data: {
        ...
        profile: {
          ...
          name: socialMedia.display_name || 'Perfil de Threads',
          url: socialMedia.profile_url || '',
          followers: socialMedia.followers || 0,
          total_threads: socialMedia.posts || 0,
        ...
        top_mentions: {
          most_positive: topPositiveMentions.map(m => ({
            ...
            thread_content: m.metadata?.thread_content
          })),
    ...
    console.log('✅ Dashboard de Threads generado exitosamente');

    return NextResponse.json(dashboard);

  } catch (error: any) {
    console.error('❌ Error generando dashboard de Threads:', error);
    return NextResponse.json(
