...
export async function POST(request: NextRequest) {
  try {
    ...
    const avgSentimentScore = totalMentions > 0
      ? allMentions.reduce((sum, m) => sum + (m.metadata?.sentiment_score || 0), 0) / totalMentions
      : 0;
    ...
    const totalVideoLikes = videos.reduce((sum, v) => sum + parseInt(v.statistics.likeCount || '0'), 0);
