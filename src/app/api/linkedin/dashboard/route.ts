...
/**
 * Dashboard de LinkedIn - Datos consolidados para visualización
 *
 * GET /api/linkedin/dashboard
 *
 * Retorna:
 * - Métricas del perfil (conexiones, posts, engagement)
...
export async function GET(request: NextRequest) {
  try {
    console.log('📊 LinkedIn Dashboard: Generando datos...');
    ...
    const dashboard = {
      ...
      data: {
        ...
        profile: {
          ...
          name: socialMedia.display_name || 'Perfil de LinkedIn',
          url: socialMedia.profile_url || '',
          connections: socialMedia.followers || 0, // LinkedIn usa "connections" en lugar de "followers"
        ...
        metrics_history: (metricsHistory || []).map(h => ({
          date: h.date,
          connections: h.followers, // Mapear followers a connections para LinkedIn
    ...
    console.log('✅ Dashboard de LinkedIn generado exitosamente');

    return NextResponse.json(dashboard);

  } catch (error: any) {
    console.error('❌ Error generando dashboard de LinkedIn:', error);
    return NextResponse.json(
