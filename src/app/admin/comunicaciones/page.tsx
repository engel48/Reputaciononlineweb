      ...
      <div className="space-y-4">
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {tab === 'notifications' && (
            <div className="ml-auto flex gap-2">
              ...
              <button onClick={purge} disabled={busy === 'purge'} className="px-3 py-2 rounded-md bg-amber-100 text-amber-700 text-sm font-medium flex items-center gap-1 hover:bg-amber-200 disabled:opacity-50">
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200 flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-[200px]">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar..." className="flex-1 px-3 py-2 rounded-md bg-gray-50 border border-gray-200 text-gray-900 text-sm placeholder-gray-500" />
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody className="divide-y divide-gray-200 text-gray-600">
                  {items.map((it) => (
                    <tr key={it.id} className="hover:bg-gray-100 align-top">
                      <td className="px-3 py-2"><div className="text-gray-900 text-xs">{it.user?.name || '—'}</div><div className="text-xs text-gray-500">{it.user?.email || ''}</div></td>
                      {tab === 'notifications' && (
                        <>
                          <td className="px-3 py-2 text-xs">{it.title}</td>
                          <td className="px-3 py-2 text-xs text-gray-500 max-w-xs">{String(it.message || '').slice(0, 120)}</td>
                      ...
                      {tab === 'alerts' && (
                        <>
                          ...
                          <td className="px-3 py-2 text-xs text-gray-500 max-w-xs">{it.keywords}</td>
                          <td className="px-3 py-2 text-center text-xs">
                            <span className={`px-2 py-0.5 rounded text-xs ${it.is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>{it.is_active ? 'activa' : 'inactiva'}</span>
                          </td>
                          <td className="px-3 py-2 text-xs whitespace-nowrap">{fmt(it.last_triggered)}</td>
                          <td className="px-3 py-2">
                            <div className="flex items-center justify-end gap-2">
                              <button onClick={() => toggleAlert(it)} disabled={busy === it.id} className="px-2 py-1 rounded bg-gray-100 text-xs hover:bg-gray-200 inline-flex items-center gap-1 disabled:opacity-50">
                      ...
                      {tab === 'subscriptions' && (
                        <>
                          ...
                          <td className="px-3 py-2 text-center"><span className="px-2 py-0.5 rounded text-xs bg-gray-100">{it.status}</span></td>
                          ...
                          <td className="px-3 py-2 text-right">
                            {it.status !== 'cancelled' && it.status !== 'canceled' ? (
                              <button onClick={() => cancelSub(it)} disabled={busy === it.id} className="px-2 py-1 rounded bg-red-100 text-red-700 text-xs hover:bg-red-200 inline-flex items-center gap-1 disabled:opacity-50">
...
function BroadcastModal({ onClose, onSent }: { onClose: () => void; onSent: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onClose}>
      <div className="bg-white border border-gray-200 rounded-xl w-full max-w-md p-5 space-y-3" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-gray-900 font-semibold flex items-center gap-2"><Send className="w-4 h-4 text-cyan-600" /> Difundir notificación</h3>
        <div className="flex gap-2">
          <select value={segment} onChange={(e) => setSegment(e.target.value as any)} className="px-3 py-2 rounded-md bg-gray-50 border border-gray-200 text-gray-900 text-sm">
            <option value="all">Todos los usuarios activos</option>
            <option value="plan">Por plan</option>
          </select>
          {segment === 'plan' && (
            <select value={plan} onChange={(e) => setPlan(e.target.value)} className="px-3 py-2 rounded-md bg-gray-50 border border-gray-200 text-gray-900 text-sm">
              {['free', 'basico', 'pro', 'enterprise'].map((p) => <option key={p} value={p}>{p}</option>)}
            </select>
          )}
        </div>
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Título" className="w-full px-3 py-2 rounded-md bg-gray-50 border border-gray-200 text-gray-900 text-sm placeholder-gray-500" />
        <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Mensaje" rows={3} className="w-full px-3 py-2 rounded-md bg-gray-50 border border-gray-200 text-gray-900 text-sm placeholder-gray-500" />
        {err && <div className="text-red-700 text-xs">{err}</div>}
        <div className="flex justify-end gap-2 pt-1">
          <button onClick={onClose} className="px-3 py-2 rounded-md bg-gray-100 text-gray-900 text-sm">Cancelar</button>
